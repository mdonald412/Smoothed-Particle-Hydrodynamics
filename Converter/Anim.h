...
namespace Azul
{
	...
	public:
		...
		bool LocallyCompressible(int index);
		bool localMin(int index);
		bool localMax(int index);
		void RemoveFrames(int index, std::vector<Azul::Frame>& inData);

		std::vector<Vec4> getClipDegrees();

		Vec4 getDegreeBend(int index);
