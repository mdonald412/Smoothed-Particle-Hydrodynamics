...
#ifdef UAV_Positions
    RWStructuredBuffer<float4> PositionsBuff : register(u2);  // (UnorderedAccessBufferSlot::BoneWorldOut)
#endif

#ifdef UAV_Neighbors
    RWStructuredBuffer<int> NeighborsBuff : register(u3);  // (UnorderedAccessBufferSlot::BoneWorldOut)
#endif

#ifdef UAV_Densities
    RWStructuredBuffer<float> DensitiesBuff : register(u4);  // (UnorderedAccessBufferSlot::BoneWorldOut)
#endif

#ifdef UAV_Pressures
    RWStructuredBuffer<float4> PressuresBuff : register(u5);  // (UnorderedAccessBufferSlot::BoneWorldOut)
#endif


#ifdef UAV_Velocities
    RWStructuredBuffer<float4> VelocitiesBuff : register(u6);  // (UnorderedAccessBufferSlot::BoneWorldOut)
#endif
...
#ifdef CBV_Instance
    cbuffer AA4 : register(b11) // (ConstantBufferSlot::csWorld)
    {
        float4 instancePos[4];
    };
#endif
